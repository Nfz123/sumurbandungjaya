            ...
            <table class="table table-bordered" id="tujuan-table">
                ...
                <tbody>
                    <tr>
                        ...
                        <td style="min-width: 20px;"><input type="number" name="qty_terima[]" class="form-control" required></td>
                        <td style="min-width: 50px;"><input type="number" name="qty_tolak[]" class="form-control" required></td>
                        <td>
                            <div class="d-flex">
                        ...
                        <td><input type="number" name="tkbm[]" class="form-control" required></td>
                        <td><input type="number" name="uangjalan[]" class="form-control uangjalan-input"  readonly required></td>
                        <td><input type="number" name="ritasi[]" class="form-control ritasi-input" readonly required></td>
                        <td>
        ...
        <div class="modal-dialog modal-dialog-scrollable">
            ...
            <div class="modal-body">
                ...
                @foreach ($tujuans as $tujuan)
                    <li class="list-group-item tujuan-item" data-nama="{{ $tujuan->namatujuan }}"  data-uangjalan="{{ $tujuan->uangjalan }}"
                            data-ritasi="{{ $tujuan->ritasi }}">

                            {{-- {{ $tujuan->namatujuan }} --}}
                            <strong>{{ $tujuan->namatujuan }}</strong>
                        <br>
                        <small class="text-muted">
                            Uang Jalan: Rp {{ number_format($tujuan->uangjalan, 0, ',', '.') }} |
                            Ritasi: Rp {{ number_format($tujuan->ritasi, 0, ',', '.') }}
                        </small>
                    </li>
                @endforeach
                </ul>
...
@push('scripts')
    ...
    <script>
        document.getElementById('add-row').addEventListener('click', function () {
            ...
            newRow.innerHTML = `
                ...
                <td><input type="number" name="tkbm[]" class="form-control" placeholder="0" required></td>
                <td><input type="number" name="uangjalan[]" class="form-control uangjalan-input" placeholder="Uang Jalan" readonly required></td>
                <td><input type="number" name="ritasi[]" class="form-control ritasi-input" placeholder="Ritasi" readonly required></td>
    ...
    <script>
        ...
        // buka modal & simpan row aktif
        $(document).on('click', '.open-modal', function () {
            activeRow = $(this).closest('tr');
            $('#modalTujuan').modal('show');
        });

        // klik tujuan di modal
        $(document).on('click', '.tujuan-item', function () {
            if (!activeRow) return;

            let nama      = $(this).data('nama');
            let uangjalan = $(this).data('uangjalan');
            let ritasi    = $(this).data('ritasi');
            // console.log('ritasi:', ritasi);
            activeRow.find('.tujuan-input').val(nama);
            activeRow.find('.uangjalan-input').val(uangjalan);
            activeRow.find('.ritasi-input').val(ritasi);

            // fokus ke qty terima
            activeRow.find('input[name="qty_terima[]"]').focus();
